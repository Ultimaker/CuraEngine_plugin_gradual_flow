...
class CuraEngineGradualFlowPluginConan(ConanFile):
    ...
    def package(self):
        ...
        copy(self, pattern=f"curaengine_plugin_gradual_flow{ext}", dst=os.path.join(self.package_folder, "res", self._cura_plugin_name, {"armv8": "arm64"}.get(str(self.settings.arch), str(self.settings.arch)), {"Macos": "Darwin"}.get(str(self.settings.os), str(self.settings.os))), src=os.path.join(self.build_folder))
